...
# 1. Instalar solo las dependencias de sistema necesarias (sin herramientas de Ada)
RUN apt-get update && \
...
# 2. Descargar e instalar Alire 2.1.0
...
# Seleccionamos GNAT y GPRBUILD. 
# Alire descargará los binarios precompilados, evitando conflictos con apt.
...
# 4. Variables de entorno para el enlazado (Crucial para AWS y librerías C)
