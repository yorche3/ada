...
Implementaciones de la [Fase 1 — Algoritmos Puros](https://yorche3.github.io/programming_languages/ROADMAP/#fase-1--algoritmos-puros--algorithms-pure-) en **Ada**: algoritmos sobre arrays con indicadores de fallo compatibles y sin excepciones.
...
> **ES:** `naive_sort` es la primera implementación homologada de esta fase en Ada. Los módulos restantes siguen el orden de la numeración canónica `05_` a `10_`.
> **EN:** `naive_sort` is the first standardized implementation of this phase in Ada. The remaining modules follow the canonical numbering from `05_` to `10_`.
